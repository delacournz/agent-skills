...
Optional add-on for the `dlc-expo` skill. Follow these steps only when the user picks **Delacour UI** as the component library (see the component-library prompt in [../SKILL.md](../SKILL.md)). **Requires Uniwind setup** ([./uniwind.md](./uniwind.md)) to be completed first, because Delacour UI is styled with Uniwind + Tailwind v4. Do not skip steps and do not paraphrase commands or code snippets, copy them as written.
...
> Delacour UI targets iOS and Android. It uses native modules (Reanimated, Gesture Handler, Keyboard Controller, Bottom Sheet), so it needs a dev-client build, not Expo Go (the base scaffold already provides one).
...
3. Add the first components. On first run `add` initialises the project (writes `native-components.json`, confirms the `withUniwindConfig` wrap, appends the `@source` globs and theme block to `src/styles/global.css`, and copies the theme and `DelacourProvider` into `src/components/ui/`), then copies the named components and installs what they need:
...
8. Check the wiring. Every check is a failure that produces no error on its own:
   ...
   Fix anything it names before writing screens. Run it again whenever a component renders unstyled, stops responding to touch, or loses classes in a release build, and do so before changing any code.
...
9. **(Recommended) Install the Delacour UI agent skill** into the new app so future agent sessions copy real components instead of writing look-alikes. It is bundled in the CLI and matches the pinned version:
   ...
   ```sh
   bunx delacour@alpha skills
   ```

   - It detects which assistants the project uses and writes the skill for each. `--agent <name>` targets one, `--agent all` targets every supported assistant, and `--list` previews without writing.
